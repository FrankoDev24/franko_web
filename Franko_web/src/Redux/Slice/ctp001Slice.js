...
/* ════════════════════════════════════════════
   AXIOS INSTANCE — Vite ENV variables
════════════════════════════════════════════ */

// ⚠️ IMPORTANT: Restart your Vite dev server after changing the .env file!
const cmsAxios = axios.create({
  baseURL: import.meta.env.VITE_CMS_BASE_URL || "https://cms.frankotrading.com",
  headers: {
...
const getProductId = (p) => {
  ...
  return (
    ...
    p.ProductID || p.product_id || p.id || p.Id || p.ID || ""
  );
};
...
export const getCTP001Products = createAsyncThunk(
  ...
  async ({ pageNumber = 1, recordPerPage = 200 } = {}, { rejectWithValue }) => {
    try {
      const res = await cmsAxios.get("/GetCTP001Products", {
        params: {
      ...
      let total = typeof explicitTotal === "number" 
        ? explicitTotal 
        : (products.length === recordPerPage ? pageNumber * recordPerPage + 1 : (pageNumber - 1) * recordPerPage + products.length);
      ...
      const candidates = ctp002Products
        .map((ctp2) => {
          ...
          return {
            ...
            similarity: getSimilarity(ctp001Name, ctp2Name),
...
export const mergeSingleCTP001WithCTP002 = createAsyncThunk(
  ...
  async ({ ctp001Product, ctp002Product }, { rejectWithValue }) => {
    try {
      ...
      if (!ctp1Id) return rejectWithValue(`Missing Sales Mate Product ID. Fields: ${Object.keys(ctp001Product || {}).join(", ")}`);
      if (!ctp2Id) return rejectWithValue(`Missing Website Product ID. Fields: ${Object.keys(ctp002Product || {}).join(", ")}`);
      ...
      // API expects an array for this endpoint
      const res = await cmsAxios.post("/MergeCTO001_N_CTP002Products", [payload]);
      ...
      if (!error.response) return rejectWithValue("Network error: Could not reach the server.");
  ...
  async (_, { rejectWithValue, getState }) => {
    try {
      ...
      if (!mergePairs.length) {
        return { success: true, message: "No exact name matches found.", products: [], nothingToMerge: true, mergePairs: [] };
      }

      const res = await cmsAxios.post("/MergeCTO001_N_CTP002Products", mergePairs);
      const rawData = res.data;
      ...
      return { success: true, message: "Products merged successfully", products: toArray(rawData), nothingToMerge: toArray(rawData).length === 0, mergePairs };
    } catch (error) {
      return rejectWithValue(toErrorPayload(error, "Failed to merge products"));
    }
  }
...
export const getMergedProducts = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const res = await cmsAxios.get("/GetMergedProducts");
...
export const placeOrder = createAsyncThunk(
  ...
  async (
    {
      cartId = "", productId, price = 0, quantity, customerId, customerName,
      contactNumber, deliveryAddress, geolocation = "345", paymentMode = "",
      paymentService = "", paymentAccountNumber = "", customerAccountType = "", bCode = DEFAULT_BCODE,
    },
    { rejectWithValue }
  ) => {
    try {
      if (!productId) throw new Error("Product ID is required.");
      if (price === undefined || price === null || Number(price) < 0) throw new Error("Price must be 0 or greater.");
      ...
      const payload = {
        cartId: String(cartId || ""), productId: String(productId), price: Number(price),
        quantity: Number(quantity), customerId: String(customerId), customerName: String(customerName),
        contactNumber: String(contactNumber), deliveryAddress: String(deliveryAddress),
        geolocation: String(geolocation || "345"), orderDate: getToday(), paymentMode: String(paymentMode),
        paymentService: String(paymentService), paymentAccountNumber: String(paymentAccountNumber || ""),
        customerAccountType: String(customerAccountType), bCode: String(bCode || DEFAULT_BCODE),
      };

      const res = await cmsAxios.post("/PlaceOrder", [payload]);
      return res.data;
    } catch (error) {
...
const getInitialLoading = () => ({
  ctp001Products: false, mergedProducts: false, mergeAction: false,
  singleMerge: false, findSimilar: false, placeOrder: false,
});

const getInitialError = () => ({
  ctp001Products: null, mergedProducts: null, mergeAction: null,
  singleMerge: null, findSimilar: null, placeOrder: null,
});
...
const ctp001Slice = createSlice({
  ...
  reducers: {
    clearCTP001Data: (state) => {
      Object.assign(state, {
        ctp001Products: [], mergedProducts: [], ctp001MergedProducts: [],
        mergedProductMap: {}, manualMerges: [], similarCandidates: [],
        mergeResult: null, orders: [], currentOrder: null,
        ctp001Pagination: { pageNumber: 1, recordPerPage: 10, total: 0 },
        loading: getInitialLoading(), error: getInitialError(),
      });
    },
    clearProducts: (state) => { state.ctp001Products = []; state.mergedProducts = []; state.ctp001MergedProducts = []; },
    clearOrders: (state) => { state.orders = []; state.currentOrder = null; },
    clearError: (state) => { state.error = getInitialError(); },
    clearSpecificError: (state, action) => {
      const key = action.payload;
      if (key && state.error[key] !== undefined) state.error[key] = null;
    },
    clearMergeResult: (state) => { state.mergeResult = null; },
    clearAllLoading: (state) => { state.loading = getInitialLoading(); },
    resetLoading: (state, action) => {
      const key = action.payload;
      if (key && state.loading[key] !== undefined) state.loading[key] = false;
    },
    setPagination: (state, action) => { state.ctp001Pagination = { ...state.ctp001Pagination, ...action.payload }; },
    ...
    clearSimilarCandidates: (state) => { state.similarCandidates = []; },
  ...
  extraReducers: (builder) => {
    builder
      .addCase(getCTP001Products.pending, (state) => { state.loading.ctp001Products = true; state.error.ctp001Products = null; })
      .addCase(getCTP001Products.fulfilled, (state, action) => {
        ...
        state.ctp001Pagination = { pageNumber: pageNumber || 1, recordPerPage: recordPerPage || 10, total: total || (products || []).length };
      })
      .addCase(getCTP001Products.rejected, (state, action) => { state.loading.ctp001Products = false; state.error.ctp001Products = action.payload || "Failed to fetch CTP001 products"; })

      .addCase(findSimilarCTP002Products.pending, (state) => { state.loading.findSimilar = true; state.error.findSimilar = null; state.similarCandidates = []; })
      .addCase(findSimilarCTP002Products.fulfilled, (state, action) => { state.loading.findSimilar = false; state.similarCandidates = action.payload?.candidates || []; })
      .addCase(findSimilarCTP002Products.rejected, (state, action) => { state.loading.findSimilar = false; state.error.findSimilar = action.payload || "Failed to find similar products"; state.similarCandidates = []; })

      .addCase(mergeSingleCTP001WithCTP002.pending, (state) => { state.loading.singleMerge = true; state.error.singleMerge = null; })
      .addCase(mergeSingleCTP001WithCTP002.fulfilled, (state, action) => {
        state.loading.singleMerge = false;
        const { ctp001ProductId, ctp002ProductId, mergedPair, message: msg } = action.payload;
        state.mergedProductMap[ctp001ProductId] = ctp002ProductId;
        state.manualMerges.push({ ctp001ProductId, ctp002ProductId, ctP001ProductName: mergedPair.ctP001ProductName, ctP002ProductName: mergedPair.ctP002ProductName, mergedAt: new Date().toISOString() });
        state.mergeResult = { success: true, message: msg || "Merged successfully", nothingToMerge: false, count: 1 };
      })
      .addCase(mergeSingleCTP001WithCTP002.rejected, (state, action) => {
        state.loading.singleMerge = false;
        state.error.singleMerge = action.payload || "Failed to merge products";
        state.mergeResult = { success: false, message: action.payload || "Failed to merge products", nothingToMerge: false, count: 0 };
      })

      .addCase(mergeCTP001AndCTP002Products.pending, (state) => { state.loading.mergeAction = true; state.error.mergeAction = null; state.mergeResult = null; })
      .addCase(mergeCTP001AndCTP002Products.fulfilled, (state, action) => {
        ...
        state.mergeResult = { success: result.success || false, message: result.message || "", nothingToMerge: result.nothingToMerge || false, count: products.length };
      })
      ...
      .addCase(getMergedProducts.pending, (state) => { state.loading.mergedProducts = true; state.error.mergedProducts = null; })
      ...
      .addCase(getMergedProducts.rejected, (state, action) => { state.loading.mergedProducts = false; state.error.mergedProducts = action.payload || "Failed to get merged products"; })

      .addCase(placeOrder.pending, (state) => { state.loading.placeOrder = true; state.error.placeOrder = null; })
      .addCase(placeOrder.fulfilled, (state, action) => {
        state.loading.placeOrder = false;
        const orderResult = action.payload;
        if (orderResult) {
          if (Array.isArray(orderResult)) { state.orders.push(...orderResult); state.currentOrder = orderResult[0]; } 
          else { state.orders.push(orderResult); state.currentOrder = orderResult; }
        }
      })
      .addCase(placeOrder.rejected, (state, action) => { state.loading.placeOrder = false; state.error.placeOrder = action.payload || "Failed to place order"; });
  },
});
...
export const {
  clearCTP001Data, clearProducts, clearOrders, clearError, clearSpecificError,
  clearMergeResult, clearAllLoading, resetLoading, setPagination, setCurrentOrder,
  clearCurrentOrder, addOrder, clearSimilarCandidates, removeManualMerge,
} = ctp001Slice.actions;
