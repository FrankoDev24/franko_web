...
const Nav = () => {
  ...
  // image cache for search results: productImage path -> blob URL
  ...
  useEffect(() => {
    try {
      ...
      if (
        parsedCustomer &&
        parsedCustomer.customerAccountNumber
      ) {
  ...
  // Load images for filteredProducts via axiosInstance
  useEffect(() => {
    if (!filteredProducts.length) return;

    const paths = new Set();
    filteredProducts.forEach((p) => {
      if (p.productImage) paths.add(p.productImage);
    });
      ...
      await Promise.allSettled(
        toLoad.map(async (imagePath) => {
          try {
            ...
            console.error(
              `Error loading search image for path ${imagePath}:`,
              err
            );
          }
        })
  ...
  const getSearchImage = (productImage) => {
    if (!productImage) return null;
    return imageUrls[productImage] || null;
  };
  ...
  return (
    <div className="sticky top-0 z-50 bg-white">
      ...
      <Navbar className="mx-auto max-w-full px-2 py-1 rounded-none shadow-md bg-white">
        ...
        {/* Mobile Search Bar */}
        <div
          className="w-full lg:hidden mt-2 relative"
          ref={searchRef}
        >
          <form
            onSubmit={handleSearchSubmit}
        ...
        <div className="h-full overflow-hidden">
          {activeSidebar === "menu" ? (
            <List>
              ...
              <ListItem
                onClick={() =>
                  closeDrawerAndNavigate("/contact")
                }
                className={`py-2 ${
                  isActive("/contact")
                    ? "text-green-500 bg-green-50"
              ...
              {currentCustomer && (
                <ListItem
                  onClick={() =>
                    closeDrawerAndNavigate("/account")
                  }
                  className={`py-2 ${
                    isActive("/account")
                      ? "text-green-500 bg-green-50"
              ...
              <ListItem
                onClick={() =>
                  closeDrawerAndNavigate("/wishlist")
                }
                className={`py-2 ${
                  isActive("/wishlist")
                    ? "text-green-500 bg-green-50"
