...
// ─── Input Field ──────────────────────────────
const Field = ({ icon: Icon, label, type = "text", placeholder, name, value, onChange, isPassword, onKeyDown }) => {
        ...
        {PASSWORD_RULES.map((rule) => {
          ...
          return (
            <span key={rule.id} className={`am-strength__rule ${ok ? "am-strength__rule--ok" : ""}`}>
              <span className="am-strength__check" style={{ borderColor: ok ? color : "#d1d5db", background: ok ? color : "transparent" }}>
                {ok && <svg viewBox="0 0 12 12" fill="none"><path d="M2.5 6L5 8.5L9.5 3.5" stroke="#fff" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></svg>}
              </span>
              {rule.label}
            </span>
          );
    ...
    <div className="am-force-overlay">
      ...
      <div className="am-force-card">
        ...
        <div className="am-force-header">
          ...
          <p className="am-force-desc">Your account requires a password update before you can continue.</p>
        </div>
        {done ? (
          ...
          <div className="am-force-body">
            ...
            <Field icon={LockClosedIcon} label="Current Password" placeholder="Enter current password" name="oldPassword" value={form.oldPassword} onChange={handle} isPassword />
            <Field icon={LockClosedIcon} label="New Password" placeholder="Enter new password" name="newPassword" value={form.newPassword} onChange={handle} isPassword />
            <StrengthMeter password={form.newPassword} />
            <Field icon={LockClosedIcon} label="Confirm Password" placeholder="Confirm new password" name="confirmPassword" value={form.confirmPassword} onChange={handle} isPassword />
            <button className="am-btn am-btn--primary" onClick={submit} disabled={loading}>
              {loading ? <><ArrowPathIcon className="am-spin" /> Updating…</> : <>Update Password <ArrowRightIcon /></>}
            </button>
          </div>
        )}
      </div>
...
const ChangePasswordPanel = ({ customer, showNotification, onClose }) => {
  ...
  const submit = async () => {
    ...
    try {
      ...
      showNotification(typeof err === "object" ? err?.message || "Failed to change password." : err || "Failed to change password.", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="am-cp">
      ...
      <div className="am-form-fields">
        <Field icon={LockClosedIcon} label="Current Password" placeholder="Current password" name="oldPassword" value={form.oldPassword} onChange={handle} isPassword />
        <Field icon={LockClosedIcon} label="New Password" placeholder="New password" name="newPassword" value={form.newPassword} onChange={handle} isPassword />
        <StrengthMeter password={form.newPassword} />
        <Field icon={LockClosedIcon} label="Confirm Password" placeholder="Confirm new password" name="confirmPassword" value={form.confirmPassword} onChange={handle} isPassword />
        <button className="am-btn am-btn--primary" onClick={submit} disabled={loading}>
          {loading ? <><ArrowPathIcon className="am-spin" /> Updating…</> : <>Update Password <ArrowRightIcon /></>}
...
const AuthModal = ({ open, onClose, onSuccess, currentCustomer }) => {
  ...
  const [signupData, setSignupData] = useState({
    customerAccountNumber: "", firstName: "", lastName: "",
    contactNumber: "", address: "", password: "",
    accountType: "customer", email: "", accountStatus: "1",
  });
  ...
  useEffect(() => {
    if (open && authMode === "signup") setSignupData((p) => ({ ...p, customerAccountNumber: uuidv4() }));
  }, [open, authMode]);
  ...
  const handleSignup = async () => {
    ...
    try {
      ...
      setLoginData({ contactNumber: signupData.contactNumber, password: signupData.password });
      setSignupData({
        customerAccountNumber: uuidv4(), firstName: "", lastName: "",
        contactNumber: "", address: "", password: "",
        accountType: "customer", email: "", accountStatus: "1",
      });
      setTimeout(() => {
        setSuccessRedirect({ show: false, title: "", message: "" });
        setAuthMode("login");
        showNotif("Registration complete!", "success");
      }, 3000);
    } catch (err) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (!result?.contactNumber) {
        showNotif("Login succeeded but data is missing.", "error");
        return;
      }
      ...
      const message = typeof err === "object" ? err?.message || "Login failed." : err || "Login failed.";
      const isAccountNotFound = typeof err === "object" && err?.isAccountNotFound === true;
      if (isAccountNotFound) {
        setRedirecting(true);
        showNotif("No account found. Redirecting to register…", "error");
        setSignupData((prev) => ({ ...prev, contactNumber: loginData.contactNumber, customerAccountNumber: uuidv4() }));
  ...
  return (
    <>
      ...
      {forcePasswordChange && pendingCustomer && (
        <ForceChangePasswordModal
          ...
          onClose={() => {
            setForcePasswordChange(false);
            setPendingCustomer(null);
            showNotif("Password change cancelled.", "error");
          }}
        />
      )}
      ...
      <div className="am-overlay" onClick={onClose}>
        ...
        <div className="am-modal" onClick={(e) => e.stopPropagation()} onKeyDown={handleKey}>
          ...
          <div className="am-content">
            {showChangePassword ? (
              ...
              <div className="am-content__inner">
                <SuccessRedirectBanner title={successRedirect.title} message={successRedirect.message} isVisible={successRedirect.show} />
                ...
                {authMode === "login" && (
                  <div className="am-form-fields">
                    ...
                    <Field
                      ...
                      isPassword
                    />
                    <button className="am-btn am-btn--primary" onClick={handleLogin} disabled={loading || redirecting}>
                      {loading ? <><ArrowPathIcon className="am-spin" /> Signing in…</> : <>Sign In <ArrowRightIcon /></>}
                    </button>
                    <div className="am-footer-links">
                      <span>Don't have an account?</span>
                      <button className="am-link" onClick={() => setAuthMode("signup")}>Register</button>
                      <span className="am-dot">·</span>
                      <button className="am-link" onClick={() => setAuthMode("guest")}>Guest</button>
                    </div>
                    {currentCustomer && (
                      <>
                        <div className="am-divider"><span /><span className="am-divider__text">Settings</span><span /></div>
                ...
                {authMode === "signup" && (
                  <div className="am-form-fields">
                    <div className="am-field-row">
                      <Field icon={UserIcon} label="First Name" placeholder="First name" name="firstName" value={signupData.firstName} onChange={(e) => setSignupData((p) => ({ ...p, [e.target.name]: e.target.value }))} />
                      <Field icon={UserIcon} label="Last Name" placeholder="Last name" name="lastName" value={signupData.lastName} onChange={(e) => setSignupData((p) => ({ ...p, [e.target.name]: e.target.value }))} />
                    </div>
                    <Field icon={PhoneIcon} label="Phone Number" placeholder="10-digit number" name="contactNumber" value={signupData.contactNumber} onChange={(e) => setSignupData((p) => ({ ...p, [e.target.name]: e.target.value }))} />
                    <Field icon={EnvelopeIcon} type="email" label="Email (optional)" placeholder="your@email.com" name="email" value={signupData.email} onChange={(e) => setSignupData((p) => ({ ...p, [e.target.name]: e.target.value }))} />
                    <Field icon={HomeIcon} label="Address" placeholder="Your address" name="address" value={signupData.address} onChange={(e) => setSignupData((p) => ({ ...p, [e.target.name]: e.target.value }))} />
                    <Field icon={LockClosedIcon} label="Password" placeholder="Create a strong password" name="password" value={signupData.password} onChange={(e) => setSignupData((p) => ({ ...p, [e.target.name]: e.target.value }))} isPassword />
                    <StrengthMeter password={signupData.password} />
                    <button className="am-btn am-btn--primary" onClick={handleSignup} disabled={loading || successRedirect.show}>
                      {loading ? <><ArrowPathIcon className="am-spin" /> Creating…</> : <>Create Account <ArrowRightIcon /></>}
                    </button>
                    <div className="am-footer-links">
                      <span>Already have an account?</span>
                      <button className="am-link" onClick={() => setAuthMode("login")}>Sign in</button>
                    </div>
                  </div>
                )}
                ...
                {authMode === "guest" && (
                  <div className="am-form-fields">
                    ...
                    />
                    <button className="am-btn am-btn--primary" onClick={handleGuest} disabled={loading || successRedirect.show}>
                      {loading ? <><ArrowPathIcon className="am-spin" /> Setting up…</> : <>Create Guest Account <ArrowRightIcon /></>}
                    </button>
                    <div className="am-footer-links">
                      <button className="am-link" onClick={() => setAuthMode("signup")}>Register instead</button>
                      <span className="am-dot">·</span>
                      <button className="am-link" onClick={() => setAuthMode("login")}>Sign in</button>
                    </div>
                  </div>
...
const STYLES = `
  ...
  .am-link:hover { color: var(--am-primary-hover); text-decoration: underline; }
  ...
  @keyframes amStrip { from { background-position: 0% } to { background-position: 100% } }
