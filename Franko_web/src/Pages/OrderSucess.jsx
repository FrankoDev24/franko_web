...
const OrderSuccess = () => {
    ...
    const handleOrderCompletion = async () => {
      try {
        ...
        const checkoutPayload = {
          ...
          customerAccountType: checkoutDetails.customerAccountType || "Customer",
          paymentService: "Mtn",
        ...
        // ✅✅✅ FIRE META PIXEL CompleteRegistration WITH VALUE
        // Only fire AFTER API calls succeed
        trackCompleteRegistration({
          value: checkoutDetails.totalAmount,
          currency: "USD", // ← Change to your actual currency (GHS, EUR, etc.)
          orderId: orderId,
  ...
  const steps = [
    { icon: PhoneCall, label: "You will receive a call from our order fulfillment team shortly" },
    { icon: Package, label: "Our team will prepare and pack your order" },
    { icon: Truck, label: "Your order will be dispatched for delivery" },
    { icon: PartyPopper, label: "Enjoy your purchase from Franko Trading!" },
  ];
  ...
  return (
    <>
      <style>{`
        ...
        @media (min-width: 768px) { .or-container { padding: 48px 24px; } }
        ...
        .or-card {
          background: #fff; border: 1px solid var(--or-border);
          border-radius: var(--or-radius); overflow: hidden;
          opacity: 0; transform: translateY(12px);
          transition: all 0.5s ease;
        }
        .or-card-visible { opacity: 1; transform: translateY(0); }

        .or-banner { background: var(--or-green); padding: 32px 24px; text-align: center; }
        ...
        @keyframes or-pulse { 0%, 100% { opacity: 0.4; } 50% { opacity: 1; } }
        ...
        .or-banner-desc {
          ...
          margin: 0; max-width: 360px; line-height: 1.5; display: inline-block;
        }
        ...
        .or-ref-card {
          ...
          border: 1px solid #bbf7d0; border-radius: var(--or-radius);
          padding: 16px 20px; margin-bottom: 24px;
          display: flex; align-items: center; justify-content: space-between; gap: 16px;
        }

        .or-ref-label {
          font-size: 10px; font-weight: 700; color: var(--or-green-mid);
          text-transform: uppercase; letter-spacing: 0.04em; margin-bottom: 4px;
        }

        .or-ref-value {
          font-size: 18px; font-weight: 900; color: var(--or-green);
          font-family: 'SF Mono', 'Fira Code', monospace, var(--or-font);
          letter-spacing: 0.02em;
        }

        .or-ref-time { font-size: 13px; font-weight: 600; color: var(--or-mid); }

        .or-steps-section { margin-bottom: 24px; }

        .or-steps-title {
          font-size: 11px; font-weight: 700; color: var(--or-light);
          text-transform: uppercase; letter-spacing: 0.04em; margin-bottom: 14px;
        }

        .or-steps-list { display: flex; flex-direction: column; gap: 0; }

        .or-step { display: flex; gap: 12px; }

        .or-step-col { display: flex; flex-direction: column; align-items: center; }

        .or-step-icon-wrap {
          width: 32px; height: 32px; border-radius: 50%;
          background: var(--or-green-light);
          display: flex; align-items: center; justify-content: center;
          flex-shrink: 0; color: var(--or-green-600);
        }

        .or-step-line {
          width: 2px; flex: 1; min-height: 12px; margin: 4px 0;
          border-radius: 1px; background: #d1fae5;
        }

        .or-step-text {
          font-size: 14px; font-weight: 500; color: var(--or-mid);
          padding-top: 5px; padding-bottom: 16px; line-height: 1.5;
        }

        .or-step-text-last { font-weight: 700; color: var(--or-green-600); padding-bottom: 0; }

        .or-actions {
          display: flex; flex-direction: column; gap: 10px; margin-bottom: 20px;
        }
        @media (min-width: 640px) { .or-actions { flex-direction: row; } }

        .or-btn {
          flex: 1; padding: 14px 20px; border: none;
          border-radius: var(--or-radius); font-size: 14px; font-weight: 700;
          cursor: pointer; transition: all 0.15s; font-family: var(--or-font);
          display: flex; align-items: center; justify-content: center; gap: 8px;
        }
        .or-btn:active { transform: scale(0.98); }

        .or-btn-primary { background: var(--or-green); color: #fff; }
        ...
        .or-btn-secondary { background: #fff; color: var(--or-mid); border: 1px solid var(--or-border); }
        .or-btn-secondary:hover {
          border-color: var(--or-green-accent); color: var(--or-green);
          background: var(--or-green-lighter); transform: translateY(-1px);
        }

        .or-divider { height: 1px; background: var(--or-border); margin-bottom: 20px; }

        .or-support { text-align: center; }

        .or-support-link {
          display: inline-flex; align-items: center; gap: 6px;
          font-size: 13px; font-weight: 600; color: var(--or-light);
          text-decoration: none; cursor: pointer; border: none;
          background: none; padding: 0; font-family: var(--or-font);
          transition: color 0.15s;
        }
        .or-support-link:hover { color: var(--or-green-600); }

        .or-support-text { text-decoration: underline; text-underline-offset: 2px; }

        .or-note {
          margin-top: 24px; padding: 16px;
          background: var(--or-bg); border: 1px solid var(--or-border);
          border-radius: var(--or-radius); text-align: center;
          opacity: 0; transform: translateY(12px);
          transition: all 0.5s ease 0.2s;
        }
        .or-note-visible { opacity: 1; transform: translateY(0); }

        .or-note-text {
          font-size: 12px; font-weight: 500; color: var(--or-light);
          line-height: 1.6; margin: 0;
        }

        .or-note-text strong { font-weight: 700; color: var(--or-green-600); }
      `}</style>
          ...
          <div className={`or-card ${isVisible ? "or-card-visible" : ""}`}>
            <div className="or-banner">
              <div className="or-banner-icon-wrap">
                ...
                <div className="or-banner-icon-inner">
                  <div style={{ width: 44, height: 44 }}><AnimatedCheck /></div>
                </div>
              </div>
            ...
            <div className="or-body">
                  ...
                  {steps.map((step, idx) => {
                    ...
                    return (
                      <div key={idx} className="or-step">
                        ...
                        <p className={`or-step-text ${isLast ? "or-step-text-last" : ""}`}>
                          {step.label}
                        </p>
                      </div>
                    );
              ...
              <div className="or-actions">
                <button onClick={() => navigate("/order-history")} className="or-btn or-btn-primary">
                  <ClipboardList style={{ width: 18, height: 18 }} />
                  View My Orders
                </button>
                <button onClick={() => navigate("/")} className="or-btn or-btn-secondary">
              ...
              <div className="or-support">
                <button onClick={() => navigate("/contact")} className="or-support-link">
                  <HelpCircle style={{ width: 15, height: 15 }} />
                  <span className="or-support-text">Need help? Contact Support</span>
                </button>
              </div>
            </div>
          </div>

          <div className={`or-note ${isVisible ? "or-note-visible" : ""}`}>
