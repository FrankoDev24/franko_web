...
// Format amount with commas and 2 decimal places (e.g., 2,000.00)
...
const Checkout = () => {
  ...
  // Image cache for cart items: imagePath -> blob URL
  ...
  // Initialize customer and delivery info
  useEffect(() => {
    if (customerData) {
      setCustomerName(
  ...
  // Load all cart images via axiosInstance and cache blob URLs
  useEffect(() => {
      ...
      await Promise.allSettled(
        toLoad.map(async (imagePath) => {
          try {
            ...
            console.error(
              `Error loading checkout image for path ${imagePath}:`,
              err
            );
          }
        })
  ...
  // Render product image using axiosInstance cache
  ...
  const initiatePayment = async (
    ...
    const payload = {
      ...
      cancellationUrl:
        "https://www.frankotrading.com/order-cancelled",
    ...
    try {
      ...
      if (result.status === "Success") {
        ...
        throw new Error(
          `Hubtel Error: ${result.message || "Unknown error"}`
        );
      }
    } catch (error) {
      throw error;
  ...
  const processDirectCheckout = async (
    ...
    try {
      await dispatchOrderCheckoutWithRetry(
        orderId,
        checkoutDetails
      );

      await dispatchOrderAddressWithRetry(
        orderId,
        addressDetails
      );
    } catch (error) {
      ...
      >
        <div className="space-y-3 mt-4">
          <p className="text-gray-600 mb-4">
            Please fill in the following required fields to
            place your order:
          </p>
