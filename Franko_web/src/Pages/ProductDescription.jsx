...
const ProductDescription = () => {
  ...
  useEffect(() => {
    if (currentProduct?.length > 0) {
      ...
      // store raw productImage path so axiosInstance can use it
      const viewedItem = {
        id: prod.productID,
  ...
  // Preload Flix Media Integration (unchanged logic)
  useEffect(() => {
    ...
    const cleanupFlixMedia = () => {
      const existingFlixScripts = document.querySelectorAll(
        'script[src*="flixfacts.com"]'
      );
      existingFlixScripts.forEach((script) => script.remove());

      const existingFlixStyles = document.querySelectorAll(
        'link[href*="flixfacts.com"], style[data-flix]'
      );
      existingFlixStyles.forEach((style) => style.remove());

      const flixContainers = document.querySelectorAll(
        "#flix-inpage, #flix-minisite, .flix-inpage, .flix-minisite"
      );
    ...
    const initFlixMedia = () => {
      ...
      container.className =
        "flix-media-isolated w-full overflow-hidden";
    ...
    return () => {
      cleanup?.();
      const containmentStyle = document.getElementById(
        "flix-media-containment"
      );
      if (containmentStyle) {
  ...
  // Load all relevant images via axiosInstance and cache as blob URLs
  useEffect(() => {
    const paths = new Set();

    // main product image
    if (currentProduct?.[0]?.productImage) {
      paths.add(currentProduct[0].productImage);
    }

    // viewed products
    viewedProducts.forEach((vp) => {
      if (vp.image) paths.add(vp.image);
    });
    ...
    if (Array.isArray(cart)) {
      cart.forEach((item) => {
        const candidate =
          item.imagePath || item.productImage || item.image;
      ...
      await Promise.allSettled(
        toLoad.map(async (rawPath) => {
          try {
            ...
            console.error(
              `Error loading product image for path ${rawPath}:`,
              err
            );
          }
        })
  ...
  }, [currentProduct, viewedProducts, cart, imageUrls]);

  // Get UI image from cache (fallback to placeholder)
  const getValidImageUrl = (imagePath) => {
    if (!imagePath) return PLACEHOLDER_IMG;
    return imageUrls[imagePath] || PLACEHOLDER_IMG;
  };
    ...
    try {
      ...
      if (cartId) {
        try {
          ...
          setCartSyncError(
            "Added to cart, but sync failed. Changes saved locally."
          );
        }
      }
  ...
  const handleQuantityChange = async (productId, newQuantity) => {
    ...
    try {
      await dispatch(
        updateCartItem({ cartId, productId, quantity: newQuantity })
      ).unwrap();
  ...
  const renderCartImage = (item) => {
    const rawPath =
      item.imagePath || item.productImage || item.image;
    const src = getValidImageUrl(rawPath);
    return (
      <img
        src={src}
  ...
  // SEO image URL (static string using backend URL, unchanged UI)
  const imageUrl = (() => {
    if (!product?.productImage)
      return "https://www.frankotrading.com/frankoIcon.png";
    const fileName = product.productImage.split("\\").pop();
    const base =
      import.meta.env.VITE_API_BASE_URL ||
      "https://smfteapi.salesmate.app";
    return `${base}/Media/Products_Images/${fileName}`;
  })();

  // UI main image from axiosInstance (blob)
  const mainImageSrc = getValidImageUrl(product.productImage);
    ...
    <div className="max-w-7xl mx-auto px-4 py-10">
      <Helmet>
        ...
        <meta
          ...
          }. High-quality and best prices available.`}
        />
        <meta
          property="og:title"
        ...
        <meta
          ...
          content={`Buy ${
            product?.productName || "this product"
          } for ₵${formatPrice?.(product?.price) || "0.00"}.`}
        />
        <meta
          property="og:image"
          content={
        ...
        <meta
          property="og:url"
          content={
            productUrl || "https://www.frankotrading.com"
          }
        />
        <meta name="twitter:card" content="summary_large_image" />
        <link
          rel="canonical"
          href={`https://www.frankotrading.com/product/${
            product?.productID ||
          ...
          <div className="flex items-center justify-between py-3 gap-4">
            ...
            <div className="flex items-center gap-3 flex-shrink-0">
              <Button
                ...
                onClick={() =>
                  handleAddToCartAndOpenSidebar(product)
                }
      ...
      >
        ...
        <div className="space-y-4">
          ...
          <div
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold shadow-sm border transition duration-200 w-max ${
              outOfStock
            ...
            <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 px-4 py-3 shadow-xl z-40 flex items-center justify-between md:hidden">
              <div className="flex gap-2 w-full">
                ...
                <button
                  ...
                  disabled={!cart || cart.length === 0}
                >
                  {/* Icon intentionally left blank in original UI */}
                </button>
              </div>
            </div>
          </div>
        ...
        >
          ...
          {flixMediaError && (
            <div className="text-center py-8 text-gray-500">
              <p>
                Unable to load additional product details at this
                time.
              </p>
            </div>
          )}
      ...
      {viewedProducts.length > 0 && (
        <section className="mt-16">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6">
            {viewedProducts.map((vp, index) => {
              const productOutOfStock = isOutOfStock(vp);
              const discount =
                vp.oldPrice > 0
                  ? Math.round(
                      ((vp.oldPrice - vp.price) / vp.oldPrice) *
                        100
                    )
                  : 0;

              const vpImage = getValidImageUrl(vp.image);
                ...
                >
                  <div className="relative overflow-hidden">
                    ...
                    <div
                      ...
                      onClick={() => navigate(`/product/${vp.id}`)}
                    >
                      <img
                        src={vpImage}
                        alt={vp.name}
                  ...
                  <div className="p-3 text-center space-y-1">
                    <h3 className="text-sm font-medium text-gray-800 line-clamp-2">
                      {vp.name || "Unnamed Product"}
                    </h3>
                    <div className="flex items-center justify-center gap-1 mt-1">
                      <span className="text-red-500 font-medium text-sm">
                        ₵{formatPrice(vp.price)}.00
                      </span>
                      {vp.oldPrice > 0 && (
                        <span className="text-xs line-through text-gray-400">
                          ₵{formatPrice(vp.oldPrice)}.00
                        </span>
                      )}
                    </div>
                  </div>
