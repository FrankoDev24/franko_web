...
const OrderDetailsModal = ({ orderId, onClose }) => {
  ...
  // Cache for product images fetched with axiosInstance (blob URLs)
  ...
  // Fetch product images via axiosInstance
  useEffect(() => {
    ...
    const loadImages = async () => {
      const toLoad = salesOrder.filter(
        (o) => o.imagePath && !imageUrls[o.imagePath]
      );
      if (!toLoad.length) return;

      await Promise.allSettled(
        toLoad.map(async (o) => {
          const imagePath = o.imagePath;
          ...
          try {
            ...
            console.error(
              `Error loading product image for order item ${o.productName}:`,
              err
            );
          }
        })
          ...
          <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
            <div className="bg-gray-50 px-4 py-2 border-b border-gray-200">
              <div className="flex items-center space-x-2">
                ...
                <span className="bg-green-100 text-green-700 px-2 py-1 rounded-full text-sm font-medium">
                  {salesOrder.length} item
                  {salesOrder.length > 1 ? 's' : ''}
                </span>
              </div>
            </div>
            ...
            <div className="p-2 space-y-4">
              {salesOrder.map((order, index) => {
                ...
                const imageUrl = imagePath ? imageUrls[imagePath] : null;
