...
const PrintableInvoice = React.forwardRef(
  ({ orderId, salesOrder, deliveryAddress }, ref) => {
    ...
    // ✅ Cache for product images loaded via axiosInstance (blob URLs)
    const [imageUrls, setImageUrls] = useState({}); // key: imagePath, value: blob URL
    ...
    useEffect(() => {
      ...
      const loadImages = async () => {
        await Promise.allSettled(
          salesOrder.map(async (order) => {
            const imagePath = order?.imagePath;
            if (!imagePath || imageUrls[imagePath]) return;
            ...
            try {
              ...
              console.error(
                `Error loading product image for invoice item ${order.productName}:`,
                err
              );
            }
          })
    ...
    }, [salesOrder]); // depend only on salesOrder
    ...
    return (
      ...
      >
        ...
        <div style={{ textAlign: 'center', marginBottom: '30px' }}>
          ...
          <Text>
            {' '}
            Opposite Roxy Cinema, Kwame Nkrumah Avenue, Accra,Ghana | Contact:
            +233 246 422 338
          </Text>
          <br />
          <Text>
        ...
        >
          ...
          <tbody>
            {salesOrder.map((order, index) => {
              const imagePath = order?.imagePath;
              const blobUrl = imagePath ? imageUrls[imagePath] : null;
